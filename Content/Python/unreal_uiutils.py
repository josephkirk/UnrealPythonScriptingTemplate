...
class EditorToolbarMenuEntry(unreal.ToolMenuEntryScript):
    ...
    def __init__(self, menu="None", section="None", name="None", label="", tool_tip="", icon=["None", "None", "None"], owner_name="None", insert_position=["None", unreal.ToolMenuInsertType.DEFAULT], advanced=["None", unreal.MultiBlockType.MENU_ENTRY, unreal.UserInterfaceActionType.BUTTON, False, False, True, False]):
        unreal.Object.__init__(self)
        self._data = unreal.ToolMenuEntryScriptData(menu, section, name, label, tool_tip, icon, owner_name, insert_position, advanced)
...
def create_python_tool_menu_entry(name, label, command_string="", entry_type=unreal.MultiBlockType.MENU_ENTRY):
    ...
    if command_string:
        menu_entry.set_string_command(unreal.ToolMenuStringCommandType.PYTHON, "python", command_string)
    return menu_entry

def create_menu_button(name, label , command_string=""):
    return create_python_tool_menu_entry(name, label, command_string)

def create_toolbar_button(name, label , section_name="", command_string="", register_button=True):
    button = create_python_tool_menu_entry(name, label , command_string, entry_type=unreal.MultiBlockType.TOOL_BAR_BUTTON)
    if register_button:
...
def create_toolbar_combo_button(name, menu, section, tool_tip="", register_button=True):
    ...
    menu_entry = unreal.ToolMenuEntry(name, type=unreal.MultiBlockType.TOOL_BAR_COMBO_BUTTON)
    if register_button:
